...
function updateProgress(searchVal, row, columns) {
    progress += 1;
    document.getElementById("searchNums").innerHTML = (progress.toString() + " of " + maxProgress.toString());
    progressBar.style.width = (progress / maxProgress) * 100 + '%';
    setTimeout(function() {
            values[searchVal] = values[searchVal] + 1;
          }, 2000);
        
        
    document.getElementById("redInfo").innerText = "Red: " + values["Red"].toString();
    document.getElementById("yellowInfo").innerText = "Yellow: " + values['Yellow'].toString();
    document.getElementById("greenInfo").innerText = "Green: " + values['Green'].toString();
}
...
const AboutPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      for (var r = range.s.r + 1; r <= range.e.r; r++) {
        ...
        const valRes = value.split(":")[0];
        var all_cols = [isbn_column, title_column, author_column];
        all_cols = all_cols.concat(remaining_columns);

        updateProgress(valRes, rowAsList(sheet, r), all_cols);
        values.push(value);
      }
  ...
  return (
    <div>
      <h1>Searching</h1>
      <h2 id="searchNums">XX of XX</h2>
      <div className="progress-bar">
        <div className="progress-bar-inner" id="progress-bar-inner" style={{ width: '60%' }}></div>
      </div>
      <table>
        <tbody>
          <tr>
            <td id="redInfo" className="titleTable">Red:</td>
            <td><button style={{ backgroundColor: 'crimson' }} className="info-button">Info</button></td>
          </tr>
          <tr>
            <td id="yellowInfo" className="titleTable">Yellow:</td>
            <td><button style={{ backgroundColor: 'gold' }} className="info-button">Info</button></td>
          </tr>
          <tr>
            <td id="greenInfo" className="titleTable">Green:</td>
